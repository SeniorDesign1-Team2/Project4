...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(MotorModel_Sp23_V21b_M, 0);
  ...
  MotorModel_Sp23_V21b_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(MotorModel_Sp23_V21b_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(MotorModel_Sp23_V21b_M, true);
    }
  }
  ...
  /* Terminate model */
  MotorModel_Sp23_V21b_terminate();
